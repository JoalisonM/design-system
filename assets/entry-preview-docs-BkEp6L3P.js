...
import { n as nn, c as cn, y as yn, l as l$1, g as gt, z, o as on, j as je, Y, a as oe, B } from "./index-B6TI3DWQ.js";
...
function generateArray({ inferredType, ast: ast2 }) {
  ...
  if (depth <= 2) {
    ...
    if (!oe(compactArray)) return l$1(compactArray);
  }
  return l$1(ARRAY_CAPTION, generateArrayCode(ast2));
}
function generateObject({ inferredType, ast: ast2 }) {
  let { depth } = inferredType;
  if (depth === 1) {
    let compactObject = generateObjectCode(ast2, true);
    if (!oe(compactObject)) return l$1(compactObject);
  }
  return l$1(OBJECT_CAPTION, generateObjectCode(ast2));
}
...
function generateFunc({ inferredType, ast: ast2 }) {
  ...
  if (identifier != null) return l$1(getPrettyIdentifier(inferredType), generateCode(ast2));
  let prettyCaption = generateCode(ast2, true);
  return oe(prettyCaption) ? l$1(FUNCTION_CAPTION, generateCode(ast2)) : l$1(prettyCaption);
}
function generateElement(defaultValue, inspectionResult) {
  ...
  if (identifier != null && !isHtmlTag(identifier)) {
    ...
    return l$1(prettyIdentifier, defaultValue);
  }
  return oe(defaultValue) ? l$1(ELEMENT_CAPTION, defaultValue) : l$1(defaultValue);
}
...
var stringResolver = (rawDefaultProp) => l$1(JSON.stringify(rawDefaultProp));
function generateReactObject(rawDefaultProp) {
  ...
  if (displayName != null) {
    ...
    return l$1(prettyIdentifier, jsx2);
  }
  if ((0, import_isString.default)(type) && isHtmlTag(type)) {
    ...
    if (!oe(jsxSummary)) return l$1(jsxSummary);
  }
  return l$1(ELEMENT_CAPTION, jsx2);
}
var objectResolver = (rawDefaultProp) => {
  ...
  if (funcName != null) {
    if (isElement) return l$1(getPrettyElementIdentifier(funcName));
    inspectionResult != null && (inspectionResult = inspectValue(rawDefaultProp.toString()));
    let { hasParams } = inspectionResult.inferredType;
    return l$1(getPrettyFuncIdentifier(funcName, hasParams));
  }
  return l$1(isElement ? ELEMENT_CAPTION : FUNCTION_CAPTION);
}, defaultResolver = (rawDefaultProp) => l$1(rawDefaultProp.toString()), DEFAULT_TYPE_RESOLVERS = { string: stringResolver, object: objectResolver, function: functionResolver, default: defaultResolver };
  ...
  try {
    switch (type.name) {
      ...
      case "arrayOf": {
        ...
        return compact != null && !B(compact) ? l$1(compact) : full ? l$1(short, full) : l$1(short);
      }
      case "func": {
        let { short, full } = generateType(type, extractedProp), summary = short, detail;
        return full && full.length < MAX_FUNC_LENGTH ? summary = full : full && (detail = toMultilineSignature(full)), l$1(summary, detail);
      }
      default:
        return null;
    }
